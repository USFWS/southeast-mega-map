...
function getOffices(cb) {
  xhr.get("./data/offices.json", function(err, res) {
    ...
    if (res.statusCode !== 200)
      return cb(new Error("Could not download offices."));
...
function getStates(cb) {
  xhr.get("./data/states.json", function(err, res) {
    if (err) return cb(err);
    if (res.statusCode !== 200)
      return cb(new Error("Could not download states."));
    return cb(null, JSON.parse(res.body));
  });
}
...
function initialize(err, data) {
  ...
  const bounds = params.state
    ? states.getBounds(params.state, "NAME")
    : offices.getBounds();
